...
class ValidationLibrary extends Component {
  checkValidation(textValue, validatorsArray) {
    for (var valid in validatorsArray) {//check validations through array
      ...
      }else if (validatorsArray[valid].name == 'pancard') {
        var re = /[A-Z]{5}[0-9]{4}[A-Z]{1}/;
        if (re.test(textValue) == false) {
          return { msg: "Invalid Format", state: false };
        }
      }
      ...
      else if (validatorsArray[valid].name == "Pincode") {
        var re = /(^\d{6}$)|(^\d{6}-\d{6}$)/;
        if (re.test(textValue) == false) {
          return { msg: "Invalid pincode", state: false };
        }
      }
      ...
      var re =/^[0-9-+()@,.!=&%{}#$*/;'""^~]*$/
        if (re.test(textValue) == false) {
          return { msg: "Please enter numeric and special characters only", state: false };
        }
      }
    }
